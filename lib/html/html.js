...
function relative_files_to_vars(TEMPLATE) {
  ...
  let files = shell_find(template_dir, "-type", "f", "-regextype", "posix-extended", "-regex", COPY_FILES, "-print");

  _.each(files, function (raw_path) {
    let file_relative_to_template = raw_path.replace(template_dir, "");
    let full_path                 = path.join(OUTPUT_DIR, template_dir.replace(INPUT_DIR, ""), file_relative_to_template);
