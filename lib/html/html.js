...
/* global reduce, is_object, global, each_x, is_length_zero, to_string, is_dir, is_empty, is_true, is_whitespace, to_default, is_plain_object, find */
...
each_x(TEMPLATES, function (filename) {
  ...
  const the_template = new TEMPLATE(filename);

  function ALTER_$(func) {
    return function () {
      TEMPLATE('update $', func(TEMPLATE('$')));
      return TEMPLATE;
    };
  }
...
function TEMPLATE(filename) {
  ...
  function current_template(key, val) { // === Function that returns state.
    switch (key) {
      ...
      case 'new-file':         return path.join(OUTPUT_DIR + '/' + _.compact([current_template('name'), arguments[1]]).join('-'));
