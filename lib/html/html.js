...
function reduce_template(THE_TEMPLATE) {
...
function relative_files_to_vars(TEMPLATE) {
  ...
  const  vars = reduce_eachs(read_key(TEMPLATE, 'globals'), files, function (raw_path) {
...
function update_with_globals(TEMPLATE) {
  const KV = reduce_eachs(read_key(TEMPLATE, 'globals'), TEMPLATE.$('global'), function (o, _i, raw) {
    const kv = tag_to_var(TEMPLATE, raw);
