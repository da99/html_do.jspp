...
/* global reduce_eachs, eachs, create_or_update_key */
  ...
  default:
    while (ARGS.length > 0) {
      ...
      switch (raw_arg) {
        ...
        default:
          log_error("!!! Unknown option: " + to_string(raw_arg));
          log_error("=== Original args: " + to_string(process.argv));
          process.exit(1);
...
function reduce_tags_paste(TEMPLATE) {
  ...
  each_x($('paste'), function (raw) {
    ...
    const PARTIAL = reduce(
      new Template(fullpath_src, read_key(TEMPLATE, 'globals'), PASTE_HTML),
      reduce_template,
      template_to_html
    );
