...
/* global reduce, read_key, to_array, is_undefined, is_function, is_plain_object, global, each_x, is_length_zero, to_string, is_dir, is_empty, is_true, is_whitespace, to_default, is_plain_object, find */
...
each_x(TEMPLATES, function (filename) {
  ...
  var THE_TEMPLATE = new Template(filename);
  const FULL_PATH    = read_key(THE_TEMPLATE, 'full_path');
  ...
  const TEMPLATE_DIR = path.dirname(FULL_PATH);
...
function tag_paste_to_markup(TEMPLATE) {
  ...
  function expand_paste_tag(raw) {
    ...
    const fullpath_src = path.join(
      path.dirname(read_key('TEMPLATE', 'full_path')), src
    );

    const PARTIAL = render_locals(
      new Template(fullpath_src)
    );
...
function relative_files_to_vars(TEMPLATE) {
  ...
  _.each(files, function (raw_path) {
    ...
    vars = create_key(vars, to_var_name(FILE_RELATIVE_TO_TEMPLATE), public_url(FULL_PATH));
  });

  return update_key(TEMPLATE, "vars", vars);
...
function render_locals(TEMPLATE) {
  ...
  const NEW_VARS = reduce_eachs(VARS, $('local, when'), function (o, _i, raw) {
    if (raw.name === 'when') {
    ...
    $(raw).remove();
    return create_or_update_key(o, k, v);
  });
  ...
  return update_key(
    update_key(TEMPLATE, 'vars', NEW_VARS),
    '$',
    NEW_$
  );
...
function json_file_path() {
  path.join(OUTPUT_DIR, "conditions.json");
}

function Template(raw_path) {
