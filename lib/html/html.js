...
function reduce_template(TEMPLATE) {
  ...
  var $ = read_key(TEMPLATE, '$');

  var CURRENT_HTML = "";
  var TAGS;
  var REDUCER;

  while ($.html() !== CURRENT_HTML) {
    CURRENT_HTML = $.html();
    TAGS         = $([
    ...
    REDUCER  = eval("reduce_tags_" + TAGS[0].name);
    TEMPLATE = REDUCER(TEMPLATE, TAGS);
    $        = TEMPLATE.$;
  } // while

    // TEMPLATE = reduce(
    //   update_key(TEMPLATE, 'tags', ),
...
function reduce_tags_paste(TEMPLATE, TAGS) {
  const $    = TEMPLATE.$;

  each_x(shift_tags(TAGS, 'paste'), function (raw) {
...
function render_locals(TEMPLATE, kv) {
  const FULL_PATH = TEMPLATE.full_path;
  const $         = TEMPLATE.$;

  const NEW_VARS = reduce_eachs(
    combine(kv || {}, TEMPLATE.globals),
    $('local, when'),
    function (o, _i, raw) {
      if (raw.name === 'when') {
...
function reduce_tags_global(TEMPLATE, tags) {
  const RAW_TAGS         = shift_tags(tags, 'global');
  const GLOBALS_IN_TEMPLATE = read_key(TEMPLATE, 'globals');
